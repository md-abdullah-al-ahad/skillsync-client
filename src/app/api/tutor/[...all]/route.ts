...
async function handleTutorRequest(request: NextRequest) {
  const url = new URL(request.url);
  const pathname = url.pathname.replace("/api/tutor", "");
  ...
  const backendUrl = `${backendBaseUrl}/tutor${pathname}${searchParams ? `?${searchParams}` : ""}`;
